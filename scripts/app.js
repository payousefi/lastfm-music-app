...
var MusicApp = function(user, key, wrap, content){
  ...
  this.go = function() {
    ...
    var queryURL = baseURL+"?method="+method+"&user=" + self.username +
                    "&api_key=" + self.apikey+"&limit="+limit+
                    "&period="+period+"&format=json";
    ...
    if (!data.error && data.topartists && data.topartists.artist.length !== 0) {
      ...
      $.each(data.topartists.artist, function(i, item) {
        ...
        html += '<a href="' + item.url + '">'+
                '<div class="artist" ' + imgsrc + 'title="' + item.name + '">'+
                '<div class="title">' + item.name +
                '<span>' + item.playcount + ' plays</span></div>'+
                '<div class="dark"></div></div></a> ';
      });
      ...
      // Load error message
      html = '<p><em>User was not found on last.fm, or there was not'+
             ' enough data from the past month available.</em></p>';
    }
...
$(document).ready(function() {
  ...
  var app = new MusicApp(username, "a99b811384974c5a5e7d6e106ac5065a", "#wrap", ".content");
  app.go();
